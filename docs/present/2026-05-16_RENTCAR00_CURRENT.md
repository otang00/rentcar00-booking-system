...
## 2026-05-18 추가 작업: Landing V2 숨김 프리뷰

### 목적
- 운영 메인 `/`을 바로 교체하지 않고 `/landing-v2` 숨김 페이지로 새 랜딩 UI를 검증한다.
- 기존 예약 기능 흐름은 유지하고, 히어로 영역에 예약 검색 카드를 레이어하는 방향을 확인한다.

### 범위
- `/landing-v2` 신규 라우트 추가.
- 기존 로고와 hero 이미지 자산 사용.
- 기존 `SearchBox`, 딜리버리 위치 모달, 검색 결과 연결 흐름 재사용.
- 모바일/PC 반응형은 landing-v2 전용 class로 격리.

### 제외
- 운영 메인 `/` 교체는 아직 하지 않는다.
- 예약/결제/알림 서버 로직은 변경하지 않는다.
- 로고 자산 자체는 변경하지 않는다.

### 검증 기준
- `/landing-v2` 접근 가능.
- 위치 선택, 일정 선택, 연령 선택, 검색 버튼 흐름이 기존 검색 기능과 동일하게 동작.
- `/` 기존 페이지는 영향 없음.
- `npm run build` 통과.

### Landing V2 refinement 기준 추가
- 히어로의 `바로 예약하기`, `예약 조회하기` CTA는 제거하고 검색 카드 중심으로 행동을 모은다.
- landing-v2 버튼 색상은 blue 계열로 통일한다.
- 텍스트 컬러는 흐리게 보이지 않도록 더 진한 색상으로 보정한다.
- 추천차량은 좌우 스크롤 캐러셀로 유지하고, 가격은 확정가가 아니라 `24시간 기준가`로 표기한다.
