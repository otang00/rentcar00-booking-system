...
### Landing V2 현재 기준
- 사용자 요청에 따라 `/landing-v2`를 모바일 우선 랜딩 시안으로 다시 운영한다.
- hero 문구는 `원하는 시간, / 원하는 장소에서 / 간편하게 예약하세요.`를 사용한다.
- hero 하단에는 카드결제/SMS/카카오 상담 카드 대신 예약 흐름 01/02/03 버튼형 카드를 배치한다.
- 검색 영역은 정적 시안이 아니라 기존 `SearchBox`를 연결해 딜리버리 지역 선택, 일정 선택, 차량 검색 기능이 실제 동작해야 한다.
- 화면 하단 floating 검색 버튼과 추천차량 검색 버튼은 `/landing-v2` 내부 실제 검색 영역(`#mv2-functional-search`)으로 이동한다.
- 검색 쿼리가 생성되면 기존 `SearchResultsSection`을 `/landing-v2`에서도 표시한다.
- 헤더 링크, 푸터, 전화/카카오 상담 링크는 실제 경로와 연결한다.
- PC에서는 hero 흐름 카드와 검색 카드가 깨지지 않도록 비율 정렬을 유지한다.
