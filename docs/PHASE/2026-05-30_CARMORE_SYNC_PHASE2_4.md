...
## 2026-05-30 카모아 전체 save 모드 전환

### 전체 save-run
- 실행: `node scripts/carmore-sync/run-carmore-reconcile-sync.js --save`
- 결과:
  - desired 52
  - actual 1
  - additions 51
...
### launchd save 모드 전환
- `.env`의 `CARMORE_SYNC_SAVE=true` 설정
- `launchctl kickstart -k gui/$(id -u)/ai.otang.carmore-reconcile-sync`
- 검증:
  - runs 3
  - last exit code 0
  - latest run: sync_mode save, status success, desired 52, actual 52, unchanged 52, failed 0

### 관리자 모니터
- 관리자 예약관리 동기화 패널의 `CARMORE` 행은 `carmore_sync_runs` 최신 이력을 표시한다.
- 최신 save 성공 상태가 표시 대상이다.

## 2026-05-30 카모아 중복/삭제 보호 보강

### 배차/사용중 차량 차단 확인
- `using_car`는 IMS normalize 단계에서 `confirmed`로 정규화된다.
- 카모아 desired도 찜카와 같은 `fetchDesiredImsReservations`를 사용하므로 active blocking status에 포함된다.
- 운영 상태 분포 확인 시 미래 예약에 `confirmed | using_car`가 존재하며 차단 대상이다.

### 보강 내용
- 기존 IMS memo holiday 회수 로직을 명시 함수로 분리했다.
- 카모아 중복 휴무 생성 오류로 보이는 메시지는 재조회 후 같은 IMS memo/date holiday를 회수한다.
- 동일 `carmoreHolidaySerial`을 다른 active mapping이 공유하면 카모아 실제 holiday 삭제를 skip한다.
- 삭제 skip 시 현재 mapping만 deleted 처리해, 남은 active 예약 차단을 유지한다.

### 의도적으로 하지 않은 것
- memo가 IMS 형식이 아닌 수동 holiday는 자동 회수하지 않는다.
- 이유: 수동 휴무를 IMS 예약용으로 오인해 삭제/변경할 위험이 있기 때문이다.

### 검증
- `npm run test:carmore-sync`: 14 pass
- `npm run test:zzimcar-sync`: 32 pass
- `CARMORE_SYNC_SAVE=false node scripts/carmore-sync/run-carmore-reconcile-sync.js`:
  - desired 52, actual 52, unchanged 52, errors 0
- `node scripts/carmore-sync/run-carmore-reconcile-sync.js --save`:
  - desired 52, actual 52, unchanged 52, errors 0
- `npm run build`: pass
